...
import pandas as pd
from common.processing import process_attendance_data
import io
import uuid
...
class AttendanceRecord(db.Model):
    id = db.Column(db.String(20), primary_key=True, default=generate_custom_id)
    # id = db.Column(db.Integer, primary_key=True)
...
def register():
    if request.method == 'POST':
        ...
        if EmployeeInfo.query.filter_by(employee_id=username).first() is None:
            flash('员工编号不存在')
...
def view_attendance(employee_id):
    if request.method == 'POST':
        # 获取年月筛选条件
        year = eval(request.form.get('year'))
        month = eval(request.form.get('month'))
    else:
        # 默认显示所有历史数据
...
# 删除考勤记录路由
@app.route('/delete_attendance/<string:attendance_id>', methods=['GET'])
@login_required
def delete_attendance(attendance_id):
    try:
        ...
        if record:
            ...
            flash('未找到该考勤记录！', 'error')
    except Exception as e:
        db.session.rollback()
        flash(f'删除考勤记录失败: {str(e)}', 'error')
    return redirect(url_for('edit_attendance', employee_id=record.employee_id))
...
def monthly_summary():
    if request.method == 'POST':
        ...
        year = eval(request.form.get('year'))
        month = eval(request.form.get('month'))
        holiday = request.form.getlist('holiday[]')
    else:
        # 默认显示当前年月
        year = datetime.now().year
        month = datetime.now().month
    ...
    start_date = datetime(year, month, 1)
    end_date = datetime(year, month + 1, 1) if month < 12 else datetime(year + 1, 1, 1)
    department_employees = AttendanceRecord.query.filter(AttendanceRecord.department == current_user.department_name, 
                                                        AttendanceRecord.attendance_date >= start_date,
                                                        AttendanceRecord.attendance_date < end_date
                                                        ).all()
    
    df = pd.DataFrame([r.__dict__ for r in department_employees])
 
    if df.shape[0] > 0:
        summary_df, max_day = process_attendance_data(df, year=year, month=month, is_holiday=holiday)
        summary_data = summary_df.to_dict(orient='records')
    else:
        summary_data = {}
        max_day = 0

    return render_template('monthly_summary.html', department=department, start_date=start_date.strftime('%Y-%m-%d'), end_date=(end_date - timedelta(days=1)).strftime('%Y-%m-%d'), summary_data=summary_data, year=year, month=month, max_day=max_day, holidays=holiday)

# 下载月读考勤路由
@app.route('/download_monthly_summary')
@login_required
def download_monthly_summary():
    year = int(request.args.get('year'))
    month = int(request.args.get('month'))
    holiday = request.args.getlist('holiday')
    ...
    end_date = datetime(year, month + 1, 1) if month < 12 else datetime(year + 1, 1, 1)
    if all == 'True':
        department_employees = AttendanceRecord.query.filter(AttendanceRecord.attendance_date >= start_date,
                                                             AttendanceRecord.attendance_date < end_date
                                                            ).all()
    else:
        department_employees = AttendanceRecord.query.filter(AttendanceRecord.department == current_user.department_name, 
    ...
    return send_file(
        ...
        mimetype='text/csv',
        as_attachment=True,
        download_name=f'monthly_summary_{year}_{month}.csv'
    )
